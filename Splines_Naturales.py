...
def mostrar_explicacion():
    ...
    def ajustar_ancho(event):
        canvas_exp.itemconfig(ventana_canvas, width=event.width)

    canvas_exp.bind("<Configure>", ajustar_ancho)
    ...
    def mover_scroll(event):
        canvas_exp.yview_scroll(int(-1 * (event.delta / 120)), "units")

    canvas_exp.bind_all("<MouseWheel>", mover_scroll)
